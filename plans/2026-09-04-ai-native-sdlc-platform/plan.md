...
spec-sha: da092484383d9c9d77e6c887b57ae2e939c5c061421d6af0a765dcf2b2fc6079
status: accepted
engineer: Behzad
author: Develito
date: 2026-09-04
...
| `.claude/hooks/plan-before-edit.sh` | Block Write/Edit without accepted plan |
...
| `releases/attestations/.gitkeep` | Attestation drop zone |
...
| T6 | Production-gate allow | Attestation with release_manager → allow (no cloud side effects) |
...
**A. plan-before-edit (PreToolUse, matcher Edit|Write)**

1. Parse tool input path.
2. If path matches allowlist → allow.
3. Else read .claude/active-intent; require plans/<id>/plan.md frontmatter status: accepted.
4. Else deny with reason pointing at engineer gate.
...
**Not allowlisted:** CLAUDE.md, REVIEW.md, bands.yaml, .claude/**, scripts/**, .github/**, tests/**, CODEOWNERS, seeded artifact content after bootstrap.
...
- If command matches promote patterns (scripts/promote.sh, gh release create, npm publish, terraform apply, or SDLC_PROMOTE=1) → require releases/attestations/<id>.yaml with non-empty release_manager: (Behzad for v1).
...
4. Spec SHA da092484… is content hash of Designito accepted attachment; after seed, update frontmatter to git blob SHA of specs/.../spec.md.
